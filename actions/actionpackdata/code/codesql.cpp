...
QScriptValue CodeSql::constructor(QScriptContext *context, QScriptEngine *engine)
{
	...
	return engine->newQObject(new CodeSql(driver), QScriptEngine::ScriptOwnership);
}

CodeSql::CodeSql(Driver driver)
	: QObject(),
	QScriptable(),
...
QScriptValue CodeSql::connect(const QScriptValue &parameters) const
{
	mDatabase->close();
...
QScriptValue CodeSql::prepare(const QString &queryString, const QScriptValue &parameters)
{
...
QScriptValue CodeSql::execute(const QString &queryString)
{
...
QScriptValue CodeSql::fetchResult(IndexStyle indexStyle)
{
	if(!mQuery.isSelect())
	{
...
QScriptValue CodeSql::disconnect() const
{
	mDatabase->close();
